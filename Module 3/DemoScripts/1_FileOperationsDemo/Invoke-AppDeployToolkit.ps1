...
function Install-ADTDeployment
{
    ...
    ## Example 1: Create C:\Temp directory
    Write-ADTLogEntry -Message "Creating C:\Temp directory"
    New-ADTFolder "C:\Temp"

    ## Example 2: Copy configuration file to C:\Temp
    Write-ADTLogEntry -Message "Copying configuration file to C:\Temp"
    Copy-ADTFile -Path "$envSupportFiles\settings.ini" -Destination "C:\Temp\settings.ini"

    ## Example 3: Copy multiple files using wildcards
    Write-ADTLogEntry -Message "Copying multiple files using wildcards"
    Copy-ADTFile -Path "$envSupportFiles\*.txt" -Destination "C:\Temp"

    ## Example 4: Copy directory recursively
    Write-ADTLogEntry -Message "Copying directory recursively"
    Copy-ADTFile -Path "$envSupportFiles" -Destination "C:\Temp\SupportFiles" -Recurse

    ## Example 5: Remove copied file
    Write-ADTLogEntry -Message "Removing copied file"
    Remove-ADTFile -Path "C:\Temp\settings.ini" -ErrorAction SilentlyContinue

    ## Example 6: Remove copied folder
    Write-ADTLogEntry -Message "Removing copied folder"
    Remove-ADTFolder -Path "C:\Temp" -ErrorAction SilentlyContinue
