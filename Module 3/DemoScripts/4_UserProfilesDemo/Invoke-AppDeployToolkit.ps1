...
function Install-ADTDeployment
{
    ...
    ## Example 1: Get all user profiles
    Write-ADTLogEntry -Message "Get all user profiles"
    $allProfiles = Get-ADTUserProfiles
    Write-ADTLogEntry -Message "Found $($allProfiles.Count) user profiles"

    ## Example 2: Get user profiles excluding system accounts
    Write-ADTLogEntry -Message "DEMO: Get user profiles excluding system accounts"
    $userProfiles = Get-ADTUserProfiles -ExcludeDefaultUser
    Write-ADTLogEntry -Message "Found $($userProfiles.Count) user profiles (excluding default user)"

    ## Example 3: Get user profiles with filter script
    Write-ADTLogEntry -Message "Get user profiles with filter script"
    $localProfiles = Get-ADTUserProfiles -FilterScript { $_.NTAccount -like "*\$env:COMPUTERNAME*" -or $_.NTAccount -like "NT AUTHORITY\*" }
    Write-ADTLogEntry -Message "Found $($localProfiles.Count) local user profiles"
