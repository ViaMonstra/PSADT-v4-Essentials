...
function Install-ADTDeployment
{
    ...
    ## Example 1: Execute Setup.exe with basic arguments
    Write-ADTLogEntry -Message "Executing Setup.exe with basic arguments"
    Start-ADTProcess -FilePath "setup.exe" -ArgumentList "/S"

    ## Example 2: Execute Setup.msi with default MSI parameters
    Write-ADTLogEntry -Message "Executing Setup.exe with basic arguments"
    Start-ADTMsiProcess -Action Install -FilePath "setup.msi"

    ## Example 3: MSI with transforms
    Write-ADTLogEntry -Message "MSI with transforms"
    Start-ADTMsiProcess -Action Install -FilePath "application.msi" -Transforms "transform.mst"

    ## Example 4: Process with child process handling
    Write-ADTLogEntry -Message "Process with child process handling"
    Start-ADTProcess -FilePath "bootstrapper.exe" -ArgumentList "/silent" -WaitForChildProcesses -KillChildProcessesWithParent -CreateNoWindow

    ## Example 5: Process with custom exit codes
    Write-ADTLogEntry -Message "Process with custom exit codes"
    Start-ADTProcess -FilePath "custom.exe" -ArgumentList "/install" -SuccessExitCodes 0, 1, 2

    ## Example 6: Uninstall MSI by product code
    Write-ADTLogEntry -Message "Uninstall MSI by product code"
    Start-ADTMsiProcess -Action Uninstall -ProductCode "{12345678-1234-1234-1234-123456789012}"

    ## Example 2: Uninstall MSI by file path
    Write-ADTLogEntry -Message "Uninstall MSI by file path"
    Start-ADTMsiProcess -Action Uninstall -FilePath "application.msi"
